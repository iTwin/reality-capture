...
class RunningJob(BaseModel):
    ...
    running_tasks: int = Field(description="Running tasks", alias="runningTasks")
    ready_tasks: int = Field(description="Ready tasks", alias="readyTasks")
...
class JobSummary(BaseModel):
    ...
    jobs_running: list[RunningJob] = Field(description="List of running jobs jobs", alias="jobsRunning")
    jobs_pending: int = Field(description="Number of pending jobs", alias="jobsPending")
...
class Milestone(BaseModel):
    ...
    end_time: Optional[datetime] = Field(default=None, escription="End time of the milestone.", alias="endTime")
...
class ExecutionOnPrem(BaseExecution):
    submit_host: datetime = Field(description="Creation date time for the job.", alias="submitHost")
...
class JobCleanInfos(BaseModel):
    deleted_job_count: int = Field(description="Number of deleted jobs", alias="deletedJobCount")


class JobFilters(BaseModel):
    include_state: Optional[list[JobState]] = Field(default=None, description="Include job state",
                                                    alias="includeState")
    exclude_state: Optional[list[JobState]] = Field(default=None, description="Exclude job state",
                                                    alias="excludeState")
    limit: Optional[int] = Field(default=100, description="Number of job per page")
...
class Job(BaseModel):
    ...
    place: int = Field(description="Place of the job.")
    processing_hosts: list[str] = Field(description="List of processing hosts for the job.", alias="processingHosts")
