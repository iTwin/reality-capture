...
class EngineManager(GenericManager):
    ...
    def get_engine(self, engine_host_name: str) -> Result[EngineDetails]:
        ...
        if row is None:
            return Result(JobDetailsError.ENGINE_NOT_FOUND, None)
    ...
    def send_signal(self, engine_host_name: str, signal: EngineSignal) -> Result[EngineDetails]:
        ...
        :return: A Result[EngineDetails] with the engine details
        """
        lock_fd = self._acquire_lock(self._db_path, timeout=5)
        try:
            cursor = self._connection.cursor()
            ...
            try:
                ...
                cursor.execute(
                    "SELECT COUNT(*) FROM Engines WHERE Hostname = ? AND EndTime IS NULL",
                    (engine_host_name,)
                )
                ...
                cursor.execute(
                    "UPDATE Engines SET Signal = Signal | ? WHERE Hostname = ? AND EndTime IS NULL",
