...
class ServiceRd:
    ...
    Args:
        ...
        project_id: ID of the project where results should be saved.
    ...
    def _create_reality_data(
        self, name: str, data_type: RealityDataType
    ) -> ReturnValue[str]:
    ...
    def _update_reality_data(self, rd_id: str, update_dict: dict) -> ReturnValue[bool]:
        json_data = json.dumps(
            {"projectId": self.project_id, "realityData": update_dict}
        )
        self._connection.request(
    ...
    def _replace_scene_references(
        ...
        with open(scene_path, "r", encoding="utf-8") as file:
            ...
            for reference in refs.findall("Reference"):
                ...
                if local_to_cloud:
                    ...
                    ret = reference_table.get_local_path_from_cloud_id(
                        ref_path.text[4:]
                    )
                    new_path = ret.value
                if ret.is_error():
        ...
        tree.write(temp_path, encoding="utf-8")
        return ReturnValue(value=True, error="")
    ...
    def init(self) -> ReturnValue[bool]:
        ...
        Returns:
            True if service was correctly started, and a potential error message.
        """
        try:
    ...
    def upload_reality_data(
        self, data_path: str, name: str, data_type: RealityDataType
    ) -> ReturnValue[str]:
        """
        ...
        self._connection.request(
            "GET",
            f"/realitydata/{ret.value}/container?projectId={self.project_id}&access=Write",
            None,
        ...
        ret_bool = self._update_reality_data(
            ret.value, update_dict={"authoring": False}
        )
        if ret_bool.is_error():
            ReturnValue(value="", error=ret.error)
        return ret
    ...
    def upload_context_scene(
        self, scene_path: str, name: str, reference_table: ReferenceTable
    ) -> ReturnValue[str]:
        """
        ...
        ret = self._replace_scene_references(
            os.path.join(scene_path, "ContextScene.xml"),
            temp_filepath,
            reference_table,
            True,
        )
        ...
        return self.upload_reality_data(
            temp_dir.name, name, RealityDataType.ContextScene
        )
    ...
    def download_reality_data(
        ...
        self._connection.request(
            "GET",
            f"/realitydata/{data_id}/container?projectId={self.project_id}&access=Read",
            None,
            self._headers_read(),
        )
        response = self._connection.getresponse()
    ...
    def download_context_scene(
        self, data_id: str, output_path: str, reference_table: ReferenceTable
    ) -> ReturnValue[bool]:
        """
        ...
        if ret.is_error():
            return ret
        # change references
        return self._replace_scene_references(
