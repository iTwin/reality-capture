...
class ServiceRda:
    ...
    def submit_job(self, settings: JobSettings, job_name: str) -> ReturnValue[str]:
        ...
        self._connection.request(
            "PATCH",
            f"/realitydataanalysis/jobs/{ret.value}",
            job_json,
    ...
    def get_job_progress(self, job_id: str) -> ReturnValue[JobProgress]:
        ...
        try:
            ...
            return ReturnValue(
                value=JobProgress(JobStatus=JobStatus.UNKNOWN, progress=-1, step=""),
                error=str(e),
            )
        self._connection.request(
            "GET",
    ...
    def get_O2DJobSettings(self, job_id: str) -> ReturnValue[O2DJobSettings]:
        ...
        if data["job"]["type"] != "objects2D":
            return ReturnValue(
                value=O2DJobSettings(), error="retrieved Job is not an Objects2D Job"
            )
    ...
    def get_O3DJobSettings(self, job_id: str) -> ReturnValue[O3DJobSettings]:
        ...
        if data["job"]["type"] != "objects3D":
            return ReturnValue(
                value=O3DJobSettings(), error="retrieved Job is not a Objects3D Job"
            )
    ...
    def get_S2DJobSettings(self, job_id: str) -> ReturnValue[S2DJobSettings]:
        ...
        if data["job"]["type"] != "segmentation2D":
            return ReturnValue(
                value=S2DJobSettings(),
                error="retrieved Job is not a Segmentation2D Job",
            )
    ...
    def get_S3DJobSettings(self, job_id: str) -> ReturnValue[S3DJobSettings]:
        ...
        if data["job"]["type"] != "segmentation3D":
            return ReturnValue(
                value=S3DJobSettings(),
                error="retrieved Job is not a Segmentation3D Job",
            )
    ...
    def get_L3DJobSettings(self, job_id: str) -> ReturnValue[L3DJobSettings]:
        ...
        if data["job"]["type"] != "lines3D":
            return ReturnValue(
                value=L3DJobSettings(), error="retrieved Job is not a Lines3D Job"
            )
        return L3DJobSettings.from_json(data["job"]["settings"])

    def get_ChangeDetectionJobSettings(
        self, job_id: str
    ) -> ReturnValue[ChangeDetectionJobSettings]:
        """
        Get settings for a given job.
        ...
        if not code.success():
            return ReturnValue(
                value=ChangeDetectionJobSettings(), error=code.error_message()
            )
        data = code.response()
        if data["job"]["type"] != "changeDetection":
            return ReturnValue(
                value=ChangeDetectionJobSettings(),
