...
def main():
    ...
    detector_reality_data = rd.upload_detector(rd_client, project_id, photo_segmentation_detector, detector_name)
    if detector_reality_data is None:
        exit(1)

    # creating settings
    settings = rda.Segmentation2DJobCreateSettings(photos_scene_id=context_scene_reality_data.id(),
                                                   photo_segmentation_detector_id=detector_reality_data.id())
    # creating job
