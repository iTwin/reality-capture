...
def run_objects2d_job(rda_client, project_id, context_scene_id, detector_id, job_name):
    settings = rda_api_sdk.Objects2DJobCreateSettings(photo_scene_id=context_scene_id,
                                                      photo_object_detector_id=detector_id)
...
def main():
    ...
    # uploading detector
    detector_reality_data = rd_api_sdk.upload_detector(rd_client, project_id, photo_object_detector, detector_name)
    if detector_reality_data is None:
        exit(1)

    # running a job
    job_state, job_id = run_objects2d_job(rda_client, project_id, context_scene_reality_data.id(), detector_reality_data.id(), job_name)
