...
export function Rdas(props: RdasProps) {
    ...
    const [jobSettings, setJobSettings] = React.useState<RDASettings.JobSettings>(new RDASettings.O2DJobSettings());
    ...
    const onJobRun = async (): Promise<void> => {
        ...
        setJobId(id);
        let currentStep = CommonData.JobState.ACTIVE;

        while(currentStep === CommonData.JobState.ACTIVE) {
            ...
            if(progress.state === CommonData.JobState.SUCCESS) {
                currentStep = CommonData.JobState.SUCCESS;
                setStep(currentStep);
            }
            else if(job.state === CommonData.JobState.CANCELLED || progress.state === CommonData.JobState.CANCELLED) {
                currentStep = CommonData.JobState.CANCELLED;
                setStep(currentStep);
            }
            else if(progress.state === CommonData.JobState.FAILED) {
                currentStep = CommonData.JobState.FAILED;
                setStep(currentStep);
            }
        ...
        if(currentStep === CommonData.JobState.SUCCESS) {
    ...
    const setO2DInput = (input: "photos" | "photoObjectDetector", value: string): void => {
        const target = new RDASettings.O2DJobSettings();
        const settings = Object.assign(target, jobSettings as RDASettings.O2DJobSettings);
    ...
    const setL3DInput = (input: "meshes" | "orientedPhotos" | "photoSegmentationDetector" | "segmentation3D" | "segmentation2D", value: string): void => {
        const target = new RDASettings.L3DJobSettings();
        const settings = Object.assign(target, jobSettings as RDASettings.L3DJobSettings);
        settings.inputs[input] = value;
        setJobSettings(settings);
    };
                ...
                <div>
                    <div className="rdas-control">
                        <SelectRealityData key={0} realityDataAccessClient={props.realityDataAccessClient} placeholder={"Select photos (context scene)"} 
                            ...
                            realityDataType={["ContextScene"]} selectedRealityData={(jobSettings as RDASettings.O2DJobSettings).inputs["photos"]} />
                    </div>
                    <div className="rdas-control">
                        <SelectRealityData key={1} realityDataAccessClient={props.realityDataAccessClient} placeholder={"Select photo object detector"} 
                            ...
                            realityDataType={["ContextDetector"]} selectedRealityData={(jobSettings as RDASettings.O2DJobSettings).inputs["photoObjectDetector"]} />
                    </div>
                </div>
            );
        case "segmentation2D":
                ...
                <div>
                    <div className="rdas-control">
                        <SelectRealityData key={0} realityDataAccessClient={props.realityDataAccessClient} placeholder={"Select photos (context scene)"} 
                            ...
                            realityDataType={["ContextScene"]} selectedRealityData={(jobSettings as RDASettings.S2DJobSettings).inputs["photos"]} />
                    </div>
                    <div className="rdas-control">
                        <SelectRealityData key={1} realityDataAccessClient={props.realityDataAccessClient} placeholder={"Select photo segmentation detector"} 
                            ...
                            realityDataType={["ContextDetector"]} selectedRealityData={(jobSettings as RDASettings.S2DJobSettings).inputs["photoSegmentationDetector"]} />
                    </div>
                </div>
            );
                ...
                <div>
                    <div className="rdas-control">
                        <SelectRealityData key={0} realityDataAccessClient={props.realityDataAccessClient} placeholder={"Select orthophoto (context scene)"}
                            ...
                            realityDataType={["ContextScene"]} selectedRealityData={(jobSettings as RDASettings.S2DJobSettings).inputs["orthophoto"]} />
                    </div>
                    <div className="rdas-control">
                        <SelectRealityData key={1} realityDataAccessClient={props.realityDataAccessClient} 
                            ...
                            selectedRealityData={(jobSettings as RDASettings.S2DJobSettings).inputs["orthophotoSegmentationDetector"]} />
                    </div>
                </div>
            );
        case "lines3D":
                ...
                <div>
                    <div className="rdas-control">
                        <SelectRealityData key={0} realityDataAccessClient={props.realityDataAccessClient} placeholder={"Select meshes (context scene)"} 
                            ...
                            realityDataType={["ContextScene"]} selectedRealityData={(jobSettings as RDASettings.L3DJobSettings).inputs["meshes"]} />
                    </div>
                    <div className="rdas-control">
                        <SelectRealityData key={2} realityDataAccessClient={props.realityDataAccessClient} 
                            ...
                            realityDataType={["ContextScene"]} selectedRealityData={(jobSettings as RDASettings.L3DJobSettings).inputs["orientedPhotos"]} />
                    </div>
                    <div className="rdas-control">
                        ...
                        <div >
                            <SelectRealityData key={3} realityDataAccessClient={props.realityDataAccessClient} placeholder={"Select photo segmentation detector"} 
                                ...
                                realityDataType={["ContextDetector"]} selectedRealityData={(jobSettings as RDASettings.L3DJobSettings).inputs["photoSegmentationDetector"]} />    
                        </div>                       
                        ...
                        <div >
                            <SelectRealityData key={4} realityDataAccessClient={props.realityDataAccessClient} placeholder={"Select segmentation2D (context scene)"} 
                                ...
                                realityDataType={["ContextScene"]} selectedRealityData={(jobSettings as RDASettings.L3DJobSettings).inputs["segmentation2D"]} />
                        </div>
                    </div>                 
                </div>
            );
    ...
    const getOutputControls = () => {
        ...
        case "objects2D":
            return (<div className="rdas-controls-group">
                <ToggleSwitch className="rdas-control" label={"objects2D"} checked={(jobSettings as RDASettings.O2DJobSettings).outputs.objects2D !== ""} 
        ...
        case "segmentation2DOrtho":
            return (<div className="rdas-controls-group">
                ...
                <ToggleSwitch className="rdas-control" label={"polygons2D"} checked={(jobSettings as RDASettings.S2DJobSettings).outputs.polygons2D !== ""} 
                    labelPosition={"left"} disabled={jobId !== ""} 
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => {setS2DOutput("polygons2D", e.target.checked);}} />
                <ToggleSwitch className="rdas-control" label={"lines2D"} checked={(jobSettings as RDASettings.S2DJobSettings).outputs.lines2D !== ""} 
                    labelPosition={"left"} disabled={jobId !== ""} 
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => {setS2DOutput("lines2D", e.target.checked);}} />
                <ToggleSwitch className="rdas-control" label={"exports"} checked={(jobSettings as RDASettings.S2DJobSettings).outputs.exportedPolygons2DSHP !== ""} 
                    labelPosition={"left"} disabled={jobId !== ""} 
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => {setS2DOutput("exportedPolygons2DSHP", e.target.checked);}} />
            </div>);
        case "lines3D":
            return (<div className="rdas-controls-group">
                <ToggleSwitch className="rdas-control" label={"lines3D"} checked={(jobSettings as RDASettings.L3DJobSettings).outputs.lines3D !== ""} 
                    labelPosition={"left"} disabled={true} 
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => {setL3DOutput("lines3D", e.target.checked);}} />  
                <ToggleSwitch className="rdas-control" label={"segmentation2D"} checked={(jobSettings as RDASettings.L3DJobSettings).outputs.segmentation2D !== ""} 
                    labelPosition={"left"} disabled={jobId !== "" || (jobSettings as RDASettings.L3DJobSettings).inputs.segmentation2D != ""} 
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => {setL3DOutput("segmentation2D", e.target.checked);}} />                         
                <ToggleSwitch className="rdas-control" label={"exports"} checked={(jobSettings as RDASettings.L3DJobSettings).outputs.exportedLines3DCesium !== ""} 
                    labelPosition={"left"} disabled={jobId !== ""} 
    ...
    const getOutputs = () => {
        ...
        case "objects2D":
            return (<>
                {(jobSettings as RDASettings.O2DJobSettings).outputs.objects2D && (
                    <LabeledInput className="rdas-control" displayStyle="inline" label="objects2D" disabled={true} 
                        value={(jobSettings as RDASettings.O2DJobSettings).outputs.objects2D === "objects2D" ? "" : (jobSettings as RDASettings.O2DJobSettings).outputs.objects2D}/>
                )}
            </>);
        case "segmentation2D":
        case "segmentation2DOrtho":
            return (<>
                {(jobSettings as RDASettings.S2DJobSettings).outputs.segmentation2D && (
                    <LabeledInput className="rdas-control" displayStyle="inline" label="segmentation2D" disabled={true} 
                        value={(jobSettings as RDASettings.S2DJobSettings).outputs.segmentation2D === "segmentation2D" 
                            ? "" : (jobSettings as RDASettings.S2DJobSettings).outputs.segmentation2D}/>
                )}
                {(jobSettings as RDASettings.S2DJobSettings).outputs.polygons2D &&(
                    <LabeledInput className="rdas-control" displayStyle="inline" label="polygons2D" disabled={true} 
                        value={(jobSettings as RDASettings.S2DJobSettings).outputs.polygons2D === "polygons2D" 
                            ? "" : (jobSettings as RDASettings.S2DJobSettings).outputs.polygons2D}/>
                )}
                {(jobSettings as RDASettings.S2DJobSettings).outputs.lines2D &&(
                    <LabeledInput className="rdas-control" displayStyle="inline" label="lines2D" disabled={true} 
                        value={(jobSettings as RDASettings.S2DJobSettings).outputs.lines2D === "lines2D" 
                            ? "" : (jobSettings as RDASettings.S2DJobSettings).outputs.lines2D}/>
                )}
        ...
        case "lines3D":
            return (<>
                {(jobSettings as RDASettings.L3DJobSettings).outputs.lines3D &&(
                    <LabeledInput className="rdas-control" displayStyle="inline" label="lines3D" disabled={true} 
                        value={(jobSettings as RDASettings.L3DJobSettings).outputs.lines3D === "lines3D" 
                            ? "" : (jobSettings as RDASettings.L3DJobSettings).outputs.lines3D}/>
                )}
                {(jobSettings as RDASettings.L3DJobSettings).outputs.segmentation2D && (
                    <LabeledInput className="rdas-control" displayStyle="inline" label="segmentation2D" disabled={true} 
                        value={(jobSettings as RDASettings.L3DJobSettings).outputs.segmentation2D === "segmentation2D" 
                            ? "" : (jobSettings as RDASettings.L3DJobSettings).outputs.segmentation2D}/>
                )}               
                {(jobSettings as RDASettings.L3DJobSettings).outputs.exportedLines3DCesium &&(
                    <LabeledInput className="rdas-control" displayStyle="inline" label="exportedLines3DCesium" disabled={true} 
                        value={(jobSettings as RDASettings.L3DJobSettings).outputs.exportedLines3DCesium === "exportedLines3DCesium" 
                            ? "" : (jobSettings as RDASettings.L3DJobSettings).outputs.exportedLines3DCesium}/>
                )}
                {(jobSettings as RDASettings.L3DJobSettings).outputs.exportedLines3DDGN &&(
                    <LabeledInput className="rdas-control" displayStyle="inline" label="exportedLines3DDGN" disabled={true} 
                        value={(jobSettings as RDASettings.L3DJobSettings).outputs.exportedLines3DDGN === "exportedLines3DDGN" 
                            ? "" : (jobSettings as RDASettings.L3DJobSettings).outputs.exportedLines3DDGN}/>
                )}
            </>);
        default:
                            ...
                            {rdasJobType === "lines3D" && (
                                <div>
                                    ...
                                    <div className="rdas-controls-group">
                                        ...
                                        <Input className="rdas-input" value={(jobSettings as RDASettings.L3DJobSettings).removeSmallComponents} size="small" 
