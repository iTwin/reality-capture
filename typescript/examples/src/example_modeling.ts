...
async function monitorJob(
  realityCaptureService: RealityCaptureService,
  jobId: string,
) {
  ...
  while (
    ...
    progress.state !== JobState.SUCCESS
  ) {
    const response = await realityCaptureService.getJobProgress(
      jobId,
      getAppropriateService(JobType.RECONSTRUCTION),
    );
    if (response.isError()) {
...
async function runFillImageProperties(
  realityCaptureService: RealityCaptureService,
  imagesRealityData: ITwinRealityData,
  fipJobName: string,
  iTwinId: string,
): Promise<string> {
  // Submit FillImageProperties job to get a context scene from the images
  let fipInputs: FillImagePropertiesInputs = {
    imageCollections: [imagesRealityData.id],
  };
  let fipOutputs = [FillImagePropertiesOutputsCreate.SCENE];
  let fipOptions: FillImagePropertiesOptions = {};
  let fipSpecs: FillImagePropertiesSpecificationsCreate = {
    inputs: fipInputs,
    outputs: fipOutputs,
    options: fipOptions,
  };
  let fipJobToSubmit: JobCreate = {
    name: fipJobName,
    specifications: fipSpecs,
    type: JobType.FILL_IMAGE_PROPERTIES,
    iTwinId: iTwinId,
  };
  const submitFipResponse =
    await realityCaptureService.submitJob(fipJobToSubmit);
  if (submitFipResponse.isError()) {
    throw new Error(
      "Failed to submit FillImageProperties job : " +
        submitFipResponse.error!.error.message,
    );
  }
  ...
  const fipPropertiesResponse = await realityCaptureService.getJob(
    fipJobId,
    getAppropriateService(JobType.FILL_IMAGE_PROPERTIES),
  );
  if (fipPropertiesResponse.isError()) {
    throw new Error(
      "Failed to retrieve FillImageProperties job properties : " +
        fipPropertiesResponse.error!.error.message,
    );
  }
  return (
    fipPropertiesResponse.value!
      .specifications as FillImagePropertiesSpecifications
  ).outputs.scene;
}

async function runCalibration(
  realityCaptureService: RealityCaptureService,
  bucketDataHandler: BucketDataHandler,
  fipOutputContextScene: string,
  calibJobName: string,
  outputPath: string,
  iTwinId: string,
  calibOptions: CalibrationOptions,
): Promise<string> {
  // Submit Calibration job to get an oriented context scene
  let calibInputs: CalibrationInputs = { scene: fipOutputContextScene };
  let calibOutputs = [
    CalibrationOutputsCreate.SCENE,
    CalibrationOutputsCreate.REPORT,
  ];
  let calibSpecs: CalibrationSpecificationsCreate = {
    inputs: calibInputs,
    outputs: calibOutputs,
    options: calibOptions,
  };
  let calibJobToSubmit: JobCreate = {
    name: calibJobName,
    specifications: calibSpecs,
    type: JobType.CALIBRATION,
    iTwinId: iTwinId,
  };
  const submitCalibResponse =
    await realityCaptureService.submitJob(calibJobToSubmit);
  if (submitCalibResponse.isError()) {
    throw new Error(
      "Failed to submit Calibration job : " +
        submitCalibResponse.error!.error.message,
    );
  }
  ...
  const calibPropertiesResponse = await realityCaptureService.getJob(
    calibJobId,
    getAppropriateService(JobType.CALIBRATION),
  );
  if (calibPropertiesResponse.isError()) {
    throw new Error(
      "Failed to retrieve Calibration job properties : " +
  ...
  const calibDownloadResponse = await bucketDataHandler.downloadData(
    iTwinId,
    path.join(outputPath, "Report"),
    chars[1],
  );
  if (calibDownloadResponse.isError()) {
    throw new Error(
      "Failed to download report " + calibDownloadResponse.error!.error.message,
    );
  }
  return (
    calibPropertiesResponse.value!.specifications as CalibrationSpecifications
...
async function runReconstruction(
  realityCaptureService: RealityCaptureService,
  realityDataHandler: RealityDataHandler,
  calibOutputContextScene: string,
  reconsJobName: string,
  outputPath: string,
  iTwinId: string,
  lasOptions: OptionsLAS,
  tiles3dOptions: Options3DTiles,
) {
  ...
  let export3dTiles: ExportCreate = {
    format: Format.THREED_TILES,
    options: tiles3dOptions,
  };
  let reconsOutputs: ReconstructionOutputsCreate = {
    exports: [exportLas, export3dTiles],
  };
  let tilingOptions: TilingOptions = {
    geometricPrecision: GeometricPrecision.EXTRA,
  };
  let reconsSpecs: ReconstructionSpecificationsCreate = {
    inputs: reconsInputs,
    outputs: reconsOutputs,
    options: tilingOptions,
  };
  let reconsJobToSubmit: JobCreate = {
    name: reconsJobName,
    specifications: reconsSpecs,
    type: JobType.RECONSTRUCTION,
    iTwinId: iTwinId,
  };
  const submitReconsResponse =
    await realityCaptureService.submitJob(reconsJobToSubmit);
  if (submitReconsResponse.isError()) {
    throw new Error(
      "Failed to submit Reconstruction job : " +
        submitReconsResponse.error!.error.message,
    );
  }
  ...
  const reconsPropertiesResponse = await realityCaptureService.getJob(
    reconsJobId,
    getAppropriateService(JobType.RECONSTRUCTION),
  );
  if (reconsPropertiesResponse.isError()) {
    throw new Error(
      "Failed to retrieve Reconstruction job properties : " +
        reconsPropertiesResponse.error!.error.message,
    );
  }
  const outputs = (
    reconsPropertiesResponse.value!
      .specifications as ReconstructionSpecifications
  ).outputs;
  for (let reconsExport of outputs.exports!) {
    const reconsDownloadResponse = await realityDataHandler.downloadData(
      reconsExport.location,
      path.join(outputPath, reconsExport.format),
      "",
      iTwinId,
    );
    if (reconsDownloadResponse.isError()) {
      throw new Error(
        "Failed to download export " +
          reconsExport.format +
          " : " +
          reconsDownloadResponse.error!.error.message,
      );
    }
  }
...
async function runModelingExample() {
  ...
  try {
    ...
    const createdRealityData = await realityDataClient.createRealityData(
      "",
      iTwinId,
      realityData,
    );
    const uploadResponse = await realityDataHandler.uploadData(
      createdRealityData.id,
      imagesPath,
      "",
      iTwinId,
    );
    if (uploadResponse.isError()) {
      throw new Error(
        "Failed to upload reality data : " +
          uploadResponse.error!.error.message,
      );
    }
    console.log("Successfully uploaded images");

    const fipOutputContextScene = await runFillImageProperties(
      realityCaptureService,
      createdRealityData,
      fipJobName,
      iTwinId,
    ...
    const calibOutputContextScene = await runCalibration(
      realityCaptureService,
      bucketDataHandler,
      fipOutputContextScene,
      calibJobName,
      outputPath,
      iTwinId,
      calibOptions,
