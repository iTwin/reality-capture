...
describe("Reality Conversion utils unit tests", () => {
    it("Settings to json", function () {
        ...
        return Promise.all([
            ...
            expect(json.options.merge).to.deep.equal(true),
            expect(json.options).to.have.property("inputSRS"),
            expect(json.options.inputSRS).to.deep.equal("EPSG:32634"),
            expect(json.options).to.have.property("outputSRS"),
            expect(json.options.outputSRS).to.deep.equal("EPSG:32634"),
        ]);
    });
